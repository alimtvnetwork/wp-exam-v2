      ...
      <DialogContent className="max-w-2xl bg-card border border-border shadow-2xl p-6 max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val as 'prompt' | 'schema' | 'import')} className="w-full space-y-4">
          ...
          <TabsContent value="prompt" className="space-y-3 m-0 focus-visible:outline-none">
            ...
            <pre className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 text-xs font-mono text-slate-200 overflow-x-auto whitespace-pre-wrap max-h-72 leading-relaxed custom-scrollbar">
              {generatedAiPrompt}
            </pre>
          ...
          <TabsContent value="schema" className="space-y-3 m-0 focus-visible:outline-none">
            ...
            <pre className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 text-xs font-mono text-emerald-400 overflow-x-auto whitespace-pre-wrap max-h-72 leading-relaxed custom-scrollbar">
              {currentQuestionJson}
            </pre>
          </TabsContent>
          ...
          <TabsContent value="import" className="space-y-3 m-0 focus-visible:outline-none">
            ...
            <textarea
              ...
              rows={8}
              className="w-full p-3 rounded-xl bg-slate-950 border border-slate-800 text-xs font-mono text-slate-100 placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary custom-scrollbar resize-none"
            />
