...
class ConditionalHelpers {
     ...
     */
    public static function returnIfDefined(mixed $value, mixed $default): mixed {
        return isDefined($value) ? $value : $default;
    }
    
    /**
     * Return value if it has content, default otherwise
     */
    public static function returnIfHasContent(mixed $value, mixed $default): mixed {
        return hasContent($value) ? $value : $default;
    }
    
    /**
     * Return first defined value from list
     */
...
class UserService {
    public function processUser(int $userId, array $options): User {
        ...
        execIf(hasContent($errors), fn() => throw new ValidationException($errors));
...
execIf(
    returnIf(isDefined($user), $user->isAdmin(), false),
    fn() => logIf(true, 'info', 'Admin action')
);

// ✅ CORRECT - Break into steps
$isAdmin = isDefined($user) && $user->isAdmin();
...
```php
// Combined usage
execIf(isDefined($user), fn() => $this->loadProfile($user));
execIf(hasContent($items), fn() => $this->processItems($items));
execIf(hasKey($options, 'debug'), fn() => $this->enableDebugMode());

logIf(isPositive($duration), 'debug', 'Operation completed', ['ms' => $duration]);

$result = returnIf(isDefined($cached), $cached, $this->fetchFresh());
```
