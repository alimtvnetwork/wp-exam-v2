...
class ConfigLoader {
    ...
    private static function getNestedValue(array $array, string $key, mixed $default): mixed {
        ...
        foreach ($keys as $k) {
            if (isFalse(is_array($value)) || isMissingKey($value, $k)) {
                return $default;
            }
            $value = $value[$k];
        }
...
class ConfigSeeder {
    ...
    public function seedIfNeeded(): void {
        ...
        if (hasMismatch($currentVersion, $newVersion)) {
    ...
    private function flatten(array $array, string $prefix = ''): array {
        ...
        foreach ($array as $key => $value) {
            $newKey = hasContent($prefix) ? "{$prefix}.{$key}" : $key;
    ...
    public static function validate(string $key, mixed $value): bool {
        ...
        return match ($schema['type']) {
            'integer' => is_int($value) 
                && (isMissingKey($schema, 'min') || $value >= $schema['min'])
                && (isMissingKey($schema, 'max') || $value <= $schema['max']),
