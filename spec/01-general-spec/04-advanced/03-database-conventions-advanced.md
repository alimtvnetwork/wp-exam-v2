...
CREATE INDEX IdxOrderUserId ON Order(UserId);

-- Frequently filtered columns
CREATE INDEX IdxUserEmail ON User(Email);
CREATE INDEX IdxOrderStatus ON Order(Status);

-- Composite for common queries
CREATE INDEX IdxOrderUserIdStatus ON Order(UserId, Status);

-- Partial index for specific conditions
CREATE INDEX IdxOrderPending 
    ON Order(CreatedAt) 
...
-- Index JSON fields
CREATE INDEX IdxUserSettingsTheme 
...
CREATE INDEX IdxAuditLogTableNameRecordId 
    ON AuditLog(TableName, RecordId);
CREATE INDEX IdxAuditLogChangedAt 
...
CREATE INDEX IdxUserRoleUserId ON UserRole(UserId);
CREATE INDEX IdxUserRoleRoleId ON UserRole(RoleId);
```
...
CREATE INDEX IdxCategoryPath ON Category(Path);
```
...
CREATE INDEX CONCURRENTLY IdxUserPhone ON User(Phone);
...
CREATE INDEX IdxLargeTableColumn ON LargeTable(Column);
```
